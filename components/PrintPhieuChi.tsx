...
import { formatCurrency, formatDateForPrint, formatCurrencyToWords, calculateInterest } from '../utils/helpers';
...
export const PrintPhieuChi: React.FC<PrintPhieuChiProps> = ({
    ...
    useEffect(() => {
        ...
        const fetchQR = async () => {
            ...
            try {
                // Use the dedicated API to get a signed token and QR image
                const res = await api.transactions.getQR(transaction.id);
        ...
        <div className="print-phieu-chi p-8 bg-white" style={{ fontFamily: 'Times New Roman, serif' }}>
            ...
            <div className="mb-4 space-y-2">
                <p className="text-sm">
                    ...
                    {transaction.household.cccd && <span className="ml-4">CCCD: {transaction.household.cccd}</span>}
                </p>
                <p className="text-sm">
